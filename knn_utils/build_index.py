...
def parse_dstore_path(dstore_path):
    ...
    Example:
    /path/to/dstore_gpt2_train_1280.arrow
    """
...
def build_index(
    ...
    max_threads = min(50, multiprocessing.cpu_count() // 2)
    ...
    for start in tqdm(range(0, len(dstore), num_keys_to_add_at_a_time)):
        ...
        logger.info(f"start: {start} end: {end} num_keys_to_add_at_a_time: {num_keys_to_add_at_a_time}")
        to_add = dstore[start:end]['keys'].astype(np.float32)
