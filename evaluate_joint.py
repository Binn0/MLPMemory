...
def main():
    ...
    if base_model.config.model_type == "gpt2":
        dimension = base_model.config.n_embd
    else:
        dimension = base_model.config.hidden_size

    config = AutoConfig.from_pretrained(args.knn_generator_path)
    if base_model.config.model_type == "gpt2":
        knn_generator = GPT2MLPModel.from_pretrained(args.knn_generator_path, config=config, input_dim=dimension, output_dim=dimension)
    elif base_model.config.model_type == "llama":
        knn_generator = LlamaMLPModel.from_pretrained(args.knn_generator_path, config=config, input_dim=dimension, output_dim=dimension)
    elif base_model.config.model_type == "mistral":
        knn_generator = MistralMLPModel.from_pretrained(args.knn_generator_path, config=config, input_dim=dimension, output_dim=dimension)

    base_model.eval()
    knn_generator.eval()

    model = MLPMemory(base_model, knn_generator, lmbda=args.lmbda, knn_temp=args.knn_temp).to("cuda")
    # --------------------------------------------------Evaluation-----------------------------------------------------------
    ...
    if args.do_test:
        ...
        for batch in tqdm(eval_dataloader, desc = "Evaluating Model"):
            with torch.no_grad():
                ...
                nonpad_tokens = shift_labels[shift_labels != -100].shape[0]

                total_loss += outputs.loss.item() * nonpad_tokens
                total_tokens += nonpad_tokens


        total_loss = torch.tensor(total_loss, device=model.device)
        total_tokens = torch.tensor(total_tokens, device=model.device)

        total_loss = accelerator.gather(total_loss).sum().item()
        total_tokens = accelerator.gather(total_tokens).sum().item()

        eval_joint = math.exp(total_loss / total_tokens)

        if accelerator.is_main_process:
            print(f"Token count: {total_tokens}")
            print(f"Joint perplexity: {eval_joint:.4f}")
