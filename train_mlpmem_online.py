...
from utils.cal_loss import kl_loss_embedding_online
...
def main():
    ...
    if arch == "gpt2":
        knn_generator = GPT2MLPModel(config=config, input_dim=dim, output_dim=dim)
    elif arch == "llama":
        knn_generator = LlamaMLPModel(config=config, input_dim=dim, output_dim=dim)
    elif arch == "mistral":
        knn_generator = MistralMLPModel(config=config, input_dim=dim, output_dim=dim)
    else:
        raise ValueError(f"Unknown architecture: {arch}")

    # Load MLP Memory Model
    model = MLPMemory(base_model, knn_generator, layer_position=-1, is_train = True)
    # -------------------------------------------------Preprocess Dataset------------------------------------------------------------
    ...
    knn_dstore.set_format(type='torch', columns=knn_dstore.column_names)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                output_logits = outputs.knn_logits # (B*T, vocab_size)

                loss, loss_kl, loss_ce = kl_loss_embedding_online(output_logits, batch, tokenizer, args, batch["knn_probs"], batch["knn_label"])
        ...
        if args.checkpointing_steps == "epoch":
            ...
            if accelerator.is_main_process:
                ...
                knn_dir = os.path.join(output_dir, "knn_generator")
                os.makedirs(knn_dir, exist_ok=True)

            accelerator.wait_for_everyone()

            accelerator.save_state(os.path.join(output_dir, "knn_generator"))
